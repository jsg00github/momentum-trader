...
"""
            ...
            if settings["notify_rsi_sell"]:
                try:
                    ...
                    df = market_data.safe_yf_download(ticker, period="2y", interval="1wk", auto_adjust=False)
                    if df.empty:
                        continue
...
# Watchlist DB Management (Now unified with momentum_trader.db)
WATCHLIST_DB = os.path.join(os.path.dirname(__file__), "momentum_trader.db")

def init_watchlist_db():
    """Initialize watchlist table in momentum_trader.db"""
...
def get_watchlist():
    conn = sqlite3.connect(WATCHLIST_DB)
    cursor = conn.cursor()
    cursor.execute("SELECT ticker, notes, created_at, hypothesis FROM watchlist ORDER BY created_at DESC")
    rows = cursor.fetchall()
    conn.close()
    return [{"ticker": r[0], "note": r[1], "added_at": r[2], "hypothesis": r[3]} for r in rows]

def add_to_watchlist(ticker, note="", hypothesis=""):
    try:
        conn = sqlite3.connect(WATCHLIST_DB)
        cursor = conn.cursor()
        # Need an entry price for the new schema
        import yfinance as yf
        ...
        cursor.execute("""
            INSERT OR REPLACE INTO watchlist (ticker, notes, hypothesis, entry_price) 
            VALUES (?, ?, ?, ?)
        """, (ticker.upper(), note, hypothesis, price))
        conn.commit()
        conn.close()
        return True
    except Exception as e:
