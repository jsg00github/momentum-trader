...
class DataCache:
    ...
    def get_stale(self, ticker: str, period: str, interval: str) -> Optional[Tuple[pd.DataFrame, datetime]]:
        """
        Get cached data regardless of age - for emergency fallback when Yahoo fails.
        Returns (DataFrame, cached_time) or None if not in cache.
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
