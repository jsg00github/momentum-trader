...
function PortfolioHistoryChart({ data }) {
        ...
        if (active && payload && payload.length) {
            return (
                <div className="bg-slate-900 border border-slate-700 p-3 rounded-lg shadow-2xl backdrop-blur-md">
                    ...
                    {payload[1] && (
                        <p className={`text-xs mt-1 ${payload[1].value >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                            {payload[1].value >= 0 ? '▲' : '▼'} ${Math.abs(payload[1].value).toLocaleString()}
                            ({processed.find(p => p.displayDate === label)?.dailyChangePct.toFixed(2)}%)
                        </p>
                    )}
                </div>
            );
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-slate-800/50 rounded-xl border border-slate-700 p-5 shadow-xl">
                <h3 className="text-slate-300 font-bold text-xs uppercase tracking-wider mb-4 flex items-center gap-2">
                    <span>📊 Daily P&L</span>
                </h3>
                <div className="h-[100px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={processed}>
                            ...
                            <Bar dataKey="dailyChange" radius={[2, 2, 0, 0]}>
    ...
    return (
        <div className="space-y-8 animate-fade-in mb-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* 6. Stock Warnings (Alerts) */}
                <AnalyticsCard title="Stock Warnings" icon="⚠️">
                    <div className="space-y-4 max-h-[240px] overflow-y-auto pr-2 custom-scrollbar">
                        {suggestions?.length > 0 ? suggestions.sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 8).map((s, i) => (
                            <div key={i} className="flex items-center justify-between gap-4 group p-2 rounded-xl hover:bg-white/5 transition-all border border-transparent hover:border-slate-700/30">
                                <div className="flex items-center gap-3">
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${s.type === 'bearish' ? 'bg-red-500/10 text-red-500' : 'bg-blue-500/10 text-blue-500'}`}>
                                        {s.type === 'bearish' ? '🚫' : '💎'}
                                    </div>
                                    <div className="flex flex-col">
                                        <span className="text-[11px] text-white/95 leading-tight">
                                            <span className="font-black text-blue-400 cursor-pointer hover:underline">{s.ticker}</span> {s.reason}
                                        </span>
                                        <span className="text-[9px] text-slate-500 font-bold uppercase tracking-tighter mt-0.5">{s.action} REQUESTED</span>
                                    </div>
                                </div>
                                <span className="text-[9px] text-slate-600 font-black font-mono shrink-0 bg-slate-900 px-2 py-1 rounded-md">{s.date || 'TODAY'}</span>
                            </div>
                        )) : (
                            <div className="flex flex-col items-center justify-center py-12 opacity-30">
                                <span className="text-4xl mb-3">🛡️</span>
                                <span className="text-[11px] font-black uppercase tracking-widest text-slate-500">All positions healthy</span>
                                <span className="text-[9px] text-slate-600 mt-1">No risk alerts detected in your holdings</span>
                            </div>
                        )}
                    </div>
                </AnalyticsCard>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-4">
                    {suggestions?.length > 0 ? (
                        <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden shadow-2xl h-full backdrop-blur-sm">
                        ...
                        <div className="bg-slate-800 rounded-2xl border border-slate-700 border-dashed flex flex-col items-center justify-center p-10 text-center h-full opacity-40">
...
function TradeJournal() {
    ...
    const [binanceStatus, setBinanceStatus] = useState({ configured: false });
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [tradesRes, metricsRes, equityRes, calRes, openAnRes, snapRes, perfRes] = await Promise.all([
                ...
                axios.get(`${API_BASE}/trades/snapshots`),
                axios.get(`${API_BASE}/trades/analytics/performance`)
            ]);
    ...
    const checkBinanceStatus = async () => {
        try {
            const res = await axios.get(`${API_BASE}/binance/status`);
            setBinanceStatus(res.data);
            if (res.data.configured) {
                const balRes = await axios.get(`${API_BASE}/binance/balances`);
                setBinanceBalances(Array.isArray(balRes.data) ? balRes.data : []);
            }
        } catch (e) {
            console.error("Binance status check failed", e);
        }
    };

    const handleBinanceSync = async () => {
        if (!binanceStatus.configured) {
            alert("Binance API key not detected in .env file.");
            return;
        }
        setSyncingBinance(true);
        try {
            const res = await axios.post(`${API_BASE}/binance/sync`);
            if (res.data.error) {
                alert(`Sync Error: ${res.data.error}`);
            } else {
                alert(`Sync Complete! Added: ${res.data.added}, Skipped: ${res.data.skipped}`);
                fetchData(); // Refresh list
                checkBinanceStatus(); // Refresh balances
            }
        } catch (e) {
...
${res.data.errors.join("\n")}`);
    ...
    return (
        <div className="p-4 container mx-auto max-w-[1600px]">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        disabled={syncingBinance || !binanceStatus.configured}
                        className={`px-4 py-2 rounded-lg font-medium transition text-sm border flex items-center gap-2 ${binanceStatus.configured
                            ? 'bg-yellow-600/20 text-yellow-500 border-yellow-600/50 hover:bg-yellow-600/30'
                            : 'bg-slate-800 text-slate-500 border-slate-700 cursor-not-allowed'
                            }`}
                        title={binanceStatus.configured ? "Sync trades from Binance" : "Binance API not configured in .env"}
                    >
                        {syncingBinance ? '🟠 Syncing...' : '🟠 Sync Binance'}
                    </button>
            ...
            {/* Binance Balances Widget */}
            {binanceStatus.configured && binanceBalances.length > 0 && (
                <div className="bg-yellow-600/5 border border-yellow-600/20 rounded-xl p-4 mb-6">
                    <div className="text-yellow-500/80 text-xs uppercase tracking-widest mb-3 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-yellow-500 animate-pulse"></span>
                        Binance Wallet Balances
                    </div>
                    <div className="flex flex-wrap gap-3">
                        {binanceBalances.map(b => (
                            <div key={b.asset} className="bg-slate-900 border border-slate-800 px-3 py-2 rounded-lg flex items-center gap-3 shadow-inner">
                                <div className="text-xs font-bold text-yellow-500">{b.asset}</div>
                                <div className="text-sm font-mono text-white">
                                    {b.total < 0.001 ? b.total.toFixed(6) : b.total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 })}
                                </div>
                            </div>
                        ))}
                    </div>
...
function TradingViewChart({ chartData, elliottWave, metrics, tradeHistory }) {
    ...
    React.useEffect(() => {
        ...
        const chart = LightweightCharts.createChart(chartContainerRef.current, {
            ...
            timeScale: { borderColor: '#475569', timeVisible: true, visible: false },
        });
        chartRef.current = chart;
        ...
        const rsiChart = LightweightCharts.createChart(rsiContainerRef.current, {
            ...
            crosshair: { mode: LightweightCharts.CrosshairMode.Normal },
            rightPriceScale: { borderColor: '#475569' },
            timeScale: { borderColor: '#475569', timeVisible: true, secondsVisible: false, visible: true },
        });
        ...
        if (chartData.some(d => d.rsi_weekly)) {
            ...
            // Add RSI Levels (70, 30, 50) - Thicker for O/B and O/S
            const rsiLevels = [
                { price: 70, color: 'rgba(239, 68, 68, 0.6)', title: 'OVERBOUGHT', lineWidth: 2 },
                { price: 30, color: 'rgba(34, 197, 94, 0.6)', title: 'OVERSOLD', lineWidth: 2 },
                { price: 50, color: 'rgba(148, 163, 184, 0.2)', title: 'NEUTRAL', lineWidth: 1 }
            ];
    ...
    }, [chartData, elliottWave, metrics, tradeHistory]);
...
function DetailView({ ticker, onClose, overrideMetrics }) {
    ...
    const handleAddToWatchlist = async () => {
        try {
            await axios.post(`${API_BASE}/watchlist`, { ticker: ticker.toUpperCase(), note: 'Added from Chart Analysis' });
    ...
    const renderAiCard = (item, type) => {
        ...
        return (
            <div
                key={item.ticker}
                onClick={() => onTickerClick(item.ticker)}
                className={`bg-slate-800 p-4 rounded-lg border ${border} mb-3 cursor-pointer hover:bg-slate-700/50 transition-all hover:scale-[1.01]`}
            >
                <div className="flex justify-between items-start mb-2">
                    <div>
