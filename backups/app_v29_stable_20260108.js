                        ...
                        <table className="w-full text-left text-[11px] whitespace-nowrap">
                            ...
                            <tbody className="divide-y divide-slate-800">
                                {sortedRows.map((row) => {
                                    const {
                                        ticker, groupTrades, avgPpc, displayShares, displayCost,
                                        totalPnl, displayPrice, totalPnlPct, daysHeld, emas, currentPrice
                                    } = row;
            ...
            {activeSubTab === 'options' && (
                ...
                <div className="space-y-6">
                    <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
                        ...
                        <button onClick={handleAnalyzeOption} disabled={analyzing} className="w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 rounded-lg transition">
                            {analyzing ? 'Analizando...' : 'Calcular Probabilidades'}
                        </button>
                    </div>

                    {optionResult && (
                        <div className="bg-slate-900 border border-slate-700 rounded-xl p-6">
                            <pre className="text-xs text-slate-300 overflow-auto">{JSON.stringify(optionResult, null, 2)}</pre>
                        </div>
                    )}
                </div>
            )}
...
// Crypto Journal Component (Placeholder for Binance API)
        ...
        <div className="p-4 container mx-auto max-w-[1600px]">
            <div className="mb-6">
                <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2 mb-4">
                    ₿ Crypto Journal
                    <span className="text-xs bg-orange-600/20 text-orange-400 px-2 py-1 rounded-full">Coming Soon</span>
                </h2>
                <p className="text-slate-400 text-sm">Integración con Binance API o carga manual de posiciones crypto.</p>
            </div>

            {/* Placeholder Cards */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
                    <p className="text-slate-500 text-xs uppercase mb-1">Total USD</p>
                    <p className="text-2xl font-bold text-orange-400">$0.00</p>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
                    <p className="text-slate-500 text-xs uppercase mb-1">Open P&L</p>
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
                    <p className="text-slate-500 text-xs uppercase mb-1">Top Coin</p>
            ...
            <div className="bg-slate-800/30 rounded-xl border border-dashed border-slate-700 p-12 text-center">
                ...
                <h3 className="text-lg font-bold text-white mb-2">No hay posiciones crypto</h3>
                <p className="text-slate-400 text-sm mb-4">Conecta tu cuenta de Binance o agrega posiciones manualmente.</p>
                <div className="flex gap-3 justify-center">
                    <button className="bg-orange-600 hover:bg-orange-500 text-white px-4 py-2 rounded-lg font-medium transition text-sm" disabled>
                        🔗 Conectar Binance
                    </button>
                    <button className="bg-slate-700 hover:bg-slate-600 text-slate-200 px-4 py-2 rounded-lg font-medium transition text-sm" disabled>
                        ➕ Agregar Manual
                    </button>
                </div>
            </div>
...
function PortfolioDashboardView() {
    ...
    const fetchMetrics = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${API_BASE}/trades/unified/metrics`);
            if (res.ok) {
    ...
    return (
        <div className="p-4 container mx-auto max-w-[1600px]">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={fetchMetrics} className="text-sm bg-slate-800 hover:bg-slate-700 border border-slate-700 px-3 py-1.5 rounded transition text-slate-400">
                    ↻ Refresh
                </button>
            </div>

            {/* Currency Selector - Simplified to CCL and ARS only */}
            <div className="flex items-center gap-4 mb-6">
                <span className="text-xs text-slate-500 uppercase">Ver en:</span>
                <div className="flex bg-slate-800 rounded-lg p-0.5 border border-slate-700">
                    <button onClick={() => setDisplayCurrency('usd_ccl')} className={`px-4 py-2 rounded text-sm font-bold transition ${displayCurrency === 'usd_ccl' ? 'bg-green-600 text-white' : 'text-slate-400 hover:text-white'}`}>💵 USD CCL</button>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div className="bg-gradient-to-br from-green-900/30 to-blue-900/30 rounded-xl p-4 border border-green-700/50">
                    <p className="text-slate-400 text-xs uppercase mb-1">Total Invertido</p>
                    <p className="text-2xl font-bold text-green-400">{currencySymbol}{totals.invested?.toLocaleString()}</p>
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
                    ...
                    <p className="text-2xl font-bold text-blue-400">{currencySymbol}{totals.current?.toLocaleString()}</p>
                </div>
                <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
                    <p className="text-slate-400 text-xs uppercase mb-1">P&L Total</p>
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
                    ...
                    <p className={`text-2xl font-bold ${totals.pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                        {totals.invested > 0 ? ((totals.pnl / totals.invested) * 100).toFixed(2) : 0}%
                    </p>
                </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4 mb-8">
                ...
                <div className="bg-slate-800/50 rounded-xl p-4 border border-blue-700/30">
                    ...
                    <div className="space-y-2 text-sm">
                        <div className="flex justify-between"><span className="text-slate-400">Invested</span><span className="text-blue-400">${metrics?.usa?.invested_usd?.toLocaleString() || 0}</span></div>
                        <div className="flex justify-between"><span className="text-slate-400">P&L</span><span className={metrics?.usa?.pnl_usd >= 0 ? 'text-green-400' : 'text-red-400'}>${metrics?.usa?.pnl_usd?.toLocaleString() || 0}</span></div>
                ...
                {/* Argentina */}
                <div className="bg-slate-800/50 rounded-xl p-4 border border-sky-700/30">
                    <div className="flex items-center gap-2 mb-3">
                        <span className="text-xl">🇦🇷</span>
                        <h4 className="font-bold text-white">Argentina</h4>
                    </div>
                    <div className="space-y-2 text-sm">
                        <div className="flex justify-between"><span className="text-slate-400">Invested (ARS)</span><span className="text-sky-400">ARS {metrics?.argentina?.invested_ars?.toLocaleString() || 0}</span></div>
                        <div className="flex justify-between"><span className="text-slate-400">Invested (CCL)</span><span className="text-green-400">${metrics?.argentina?.invested_usd_ccl?.toLocaleString() || 0}</span></div>
                        <div className="flex justify-between"><span className="text-slate-400">P&L (ARS)</span><span className={metrics?.argentina?.pnl_ars >= 0 ? 'text-green-400' : 'text-red-400'}>ARS {metrics?.argentina?.pnl_ars?.toLocaleString() || 0}</span></div>
                        <div className="flex justify-between"><span className="text-slate-400">P&L (CCL)</span><span className={metrics?.argentina?.pnl_usd_ccl >= 0 ? 'text-green-400' : 'text-red-400'}>${metrics?.argentina?.pnl_usd_ccl?.toLocaleString() || 0}</span></div>
                ...
                {/* Crypto */}
                <div className="bg-slate-800/50 rounded-xl p-4 border border-orange-700/30">
                    <div className="flex items-center gap-2 mb-3">
                        <span className="text-xl">₿</span>
                        <h4 className="font-bold text-white">Crypto</h4>
                        <span className="text-xs bg-orange-600/20 text-orange-400 px-2 py-0.5 rounded-full">Coming Soon</span>
                    </div>
                    <div className="space-y-2 text-sm">
                        <div className="flex justify-between"><span className="text-slate-400">Invested</span><span className="text-orange-400">$0</span></div>
                        <div className="flex justify-between"><span className="text-slate-400">P&L</span><span className="text-slate-500">$0</span></div>
                        <div className="flex justify-between"><span className="text-slate-400">Posiciones</span><span className="text-white">0</span></div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-slate-800/50 rounded-xl overflow-hidden border border-slate-700">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr className="border-t border-slate-700">
                            <td className="p-3 text-slate-300 font-medium">Total P&L</td>
                        ...
                        <div>
                            ...
                            <div className="px-3 space-y-1">
                                ...
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 rounded-lg flex items-center gap-3 transition-all duration-200 group ${view === 'argentina' ? 'bg-sky-600 text-white shadow-lg shadow-sky-900/20 font-bold' : 'hover:bg-slate-900 text-slate-400 hover:text-white'}`}
                                >
                                    <span className="text-xl">🇦🇷</span>
                                    <span className="hidden lg:block">Argentina</span>
                                </button>
                                <button
                                    onClick={() => setView('crypto')}
                                    className={`w-full text-left px-4 py-3 rounded-lg flex items-center gap-3 transition-all duration-200 group ${view === 'crypto' ? 'bg-orange-600 text-white shadow-lg shadow-orange-900/20 font-bold' : 'hover:bg-slate-900 text-slate-400 hover:text-white'}`}
                                >
                                    <span className="text-xl">₿</span>
                                    <span className="hidden lg:block">Crypto</span>
                                </button>
                                <button
                                    onClick={() => setView('portfolio')}
                                    className={`w-full text-left px-4 py-3 rounded-lg flex items-center gap-3 transition-all duration-200 group ${view === 'portfolio' ? 'bg-gradient-to-r from-green-600 to-blue-600 text-white shadow-lg font-bold' : 'hover:bg-slate-900 text-slate-400 hover:text-white'}`}
                                >
                                    <span className="text-xl">📊</span>
                                ...
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 rounded-lg flex items-center gap-3 transition-all duration-200 group ${view === 'sharpe' ? 'bg-purple-600 text-white shadow-lg shadow-purple-900/20 font-bold' : 'hover:bg-slate-900 text-slate-400 hover:text-white'}`}
                                >
                                    <span className="text-xl">📈</span>
                                    <span className="hidden lg:block">Sharpe</span>
                                </button>
                                <button
                    ...
                    {!selectedTicker && (
                        <div className="sticky top-0 z-10 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 px-8 py-4 flex justify-between items-center">
                            <h1 className="text-xl font-bold text-white uppercase tracking-wider">
                                ...
                                {view === 'journal' && 'Journal USA 📊'}
                                {view === 'watchlist' && 'Watchlist & Alerts'}
                                {view === 'charts' && 'Advanced Charts'}
                                {view === 'argentina' && '🇦🇷 Journal Argentina'}
                                {view === 'crypto' && '₿ Journal Crypto'}
                                {view === 'portfolio' && '📈 Portfolio Dashboard'}
                                {view === 'sharpe' && '📊 Sharpe Portfolio Builder'}
