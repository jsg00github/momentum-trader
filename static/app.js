    ...
    const testAlert = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                alert("âŒ Failed to send test alert. Check BOT_TOKEN and chat_id");
                    ...
                    <div className="space-y-6">
                        ...
                        {activeTab === 'active' && (() => {
                            ...
                            sortedRows.forEach(row => {
                                ...
                                if (price === 0) return;
                                const sl  = parseFloat(row.groupTrades[0]?.stop_loss);
                                const tp1 = parseFloat(row.groupTrades[0]?.target);
                                const tp2 = parseFloat(row.groupTrades[0]?.target2);
                                const tp3 = parseFloat(row.groupTrades[0]?.target3);
                                // SL: price at or below stop loss
                                if (sl > 0 && price <= sl) slHits.push({ ticker: row.ticker, price, sl, pct: ((price - sl) / sl * 100).toFixed(1) });
                                // TP hits
                                if (tp3 > 0 && price >= tp3) tpHits.push({ ticker: row.ticker, price, target: tp3, label: 'T3', pct: ((price - tp3) / tp3 * 100).toFixed(1) });
                                else if (tp2 > 0 && price >= tp2) tpHits.push({ ticker: row.ticker, price, target: tp2, label: 'T2', pct: ((price - tp2) / tp2 * 100).toFixed(1) });
                                else if (tp1 > 0 && price >= tp1) tpHits.push({ ticker: row.ticker, price, target: tp1, label: 'T1', pct: ((price - tp1) / tp1 * 100).toFixed(1) });
                            });
                            if (slHits.length === 0 && tpHits.length === 0) return null;
                            return (
                                <div className="mb-3 p-3 rounded-xl border border-slate-700 bg-slate-900/80 backdrop-blur-sm shadow-lg">
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">🔔 Alertas Activas</span>
                                        <span className="text-[9px] text-slate-600">· basado en precios en vivo</span>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        {slHits.map(h => (
                                            <div key={`sl-${h.ticker}`} className="flex items-center gap-1.5 bg-red-950/70 border border-red-700 text-red-300 px-2.5 py-1.5 rounded-lg text-[11px] font-bold shadow animate-pulse">
                                                <span className="text-red-400">⚠️</span>
                                                <span>{h.ticker}</span>
                                                <span className="text-red-500 font-mono">SL</span>
                                                <span className="text-slate-400 font-normal text-[10px]">${h.price.toFixed(2)} ≤ ${h.sl.toFixed(2)}</span>
                                                <span className="text-red-400 font-mono text-[10px]">({h.pct}%)</span>
                                            </div>
                                        ))}
                                        {tpHits.map(h => (
                                            <div key={`tp-${h.ticker}`} className="flex items-center gap-1.5 bg-green-950/70 border border-green-600 text-green-300 px-2.5 py-1.5 rounded-lg text-[11px] font-bold shadow">
                                                <span className="text-green-400">🎯</span>
                                                <span>{h.ticker}</span>
                                                <span className="text-green-400 font-mono">{h.label}</span>
                                                <span className="text-slate-400 font-normal text-[10px]">${h.price.toFixed(2)} ≥ ${h.target.toFixed(2)}</span>
                                                <span className="text-green-400 font-mono text-[10px]">(+{h.pct}%)</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="bg-slate-900 border border-slate-700 overflow-x-auto overflow-y-auto max-h-[80vh] rounded-lg shadow-xl">
                            ...
                            <table className="w-full text-left text-[11px] whitespace-nowrap">
                                ...
                                <thead className="bg-[#0f172a] text-slate-400 uppercase font-bold border-b border-slate-600 select-none text-[10px] sticky top-0 z-20">
...
function Scanner({ onTickerClick }) {
    ...
    const [scannerTab, setScannerTab] = useState('all'); // 'all' | 'watchlist'
    ...
    const sortArrow = (key) => {
        ...
        if (sortConfig.key !== key) return <span className="text-slate-600 ml-0.5">⇅</span>;


        return <span className="text-blue-400 ml-0.5">{sortConfig.direction === 'asc' ? '▲' : '▼'}</span>;
    ...
    // Watchlist Candidate filter


    const isWatchlistCandidate = (row) => {
        if (!row) return false;

        const rsiOk = (row.rsi || 0) >= 48;
        const phaseOk = ['green', 'blue', 'yellow', 'pink'].includes(row.rsi_color);

        return rsiOk && phaseOk;
    };
    ...
    const sortedResults = useMemo(() => {
        ...
        Object.keys(columnFilters).forEach(key => {
            ...
            if (f.type === 'category' && f.values && f.values.length > 0) {
                sortableItems = sortableItems.filter(item => {
                    let val = item[key];
                    // Unpack nested
                    if (key === 'stage') val = item.stage?.stage || 0;
                    if (key === 'rsi_color') {
                        const m = { green: 'Strong', blue: 'Accum', yellow: 'Pull↑', orange: 'Pull↓', pink: 'Corr', red: 'Bear', gray: '-' };
                        val = m[val] || '-';
                    }
                    if (key === 'stars') val = '⭐'.repeat(val || 1);
                ...
                sortableItems = sortableItems.filter(item => {
                    let val = item[key];
                    // Unpack nested
                    if (key === 'range_52w') val = item.high_52w_dist_pct;
                    if (key === 'udvr') val = item.pressure_gauge?.udvr_normalized;
                    if (key === 'vcp_tightness') val = item.vcp_metrics?.tightness_pct;
                    if (key === 'macd_m') val = item.macd_m?.macd;
        ...
        if (scannerTab === 'watchlist') {


            sortableItems = sortableItems.filter(isWatchlistCandidate);
        ...
        if (sortConfig.key !== null) {
            ...
            sortableItems.sort((a, b) => {
                ...
                if (sortConfig.key === 'stage') { aVal = a.stage?.stage || 0; bVal = b.stage?.stage || 0; }
                if (sortConfig.key === 'range_52w') { aVal = a.high_52w_dist_pct || 999; bVal = b.high_52w_dist_pct || 999; }
                if (sortConfig.key === 'udvr') { aVal = a.pressure_gauge?.udvr_normalized || 0; bVal = b.pressure_gauge?.udvr_normalized || 0; }
                if (sortConfig.key === 'vcp_tightness') { aVal = a.vcp_metrics?.tightness_pct || 999; bVal = b.vcp_metrics?.tightness_pct || 999; }
    ...
    }, [results, sortConfig, showAboveSMA, scannerTab]);





    // Count watchlist candidates across all results


    const watchlistCount = useMemo(() => results.filter(isWatchlistCandidate).length, [results]);
    ...
    const handleDownloadPDF = () => {
        ...
        try {
            ...
            const tableColumn = ["Ticker", "3M%", "2M%", "1M%", "1W%", "Score", "Price", "RSI", "Phase", "52W%", "VCP", "MACD", "EMA60", "DMI (+/-)", "Vol Ratio"];
            ...
            sortedResults.forEach(ticket => {
                ...
                const labelMap = { green: 'Strong', blue: 'Accum', yellow: 'Pull↑', orange: 'Pull↓', pink: 'Corr', red: 'Bear', gray: '-' };
                ...
                const ticketData = [
                    ...
                    ticket.ticker,
                    ticket.perf_3m != null ? `${ticket.perf_3m}%` : '-',
                    ticket.perf_2m != null ? `${ticket.perf_2m}%` : '-',
                    ticket.perf_1m != null ? `${ticket.perf_1m}%` : '-',
                    ticket.perf_1w != null ? `${ticket.perf_1w}%` : '-',


                    ticket.momentum_score || 0,


                    ticket.price?.toFixed(2),


                    ticket.rsi?.toFixed(1),


                    labelMap[ticket.rsi_color] || '-',


                    ticket.high_52w_dist_pct != null ? `${ticket.high_52w_dist_pct}%` : '-',
                    ticket.vcp_metrics?.tightness_pct != null ? `T: ${ticket.vcp_metrics.tightness_pct.toFixed(1)}%` : '-',


                    ticket.macd_d?.toFixed(2),


                    ticket.ema60_d?.toFixed(0),
    ...
    const handleDownloadExcel = () => {
        ...
        try {
            ...
            const dataToExport = sortedResults.map(ticket => ({
                ...
                "Phase": labelMap[ticket.rsi_color] || '-',


                "52W High %": ticket.high_52w_dist_pct,
                ...
                "Watchlist": isWatchlistCandidate(ticket) ? "YES" : ""
    ...
    const toggleFilterMenu = (e, key) => {
        e.stopPropagation();
        setActiveFilterMenu(activeFilterMenu === key ? null : key);
    };

    const applyFilter = (key, filterData) => {
        setColumnFilters(prev => ({ ...prev, [key]: filterData }));
    };
    ...
    const Th = ({ k, children, className = '', filterType = null, filterOptions = [] }) => {
        const isActive = !!columnFilters[k];
        const showMenu = activeFilterMenu === k;
        const currentFilter = columnFilters[k] || {};

        return (
            <th className={`px-1 py-1.5 align-middle select-none whitespace-nowrap relative ${className}`}>
                <div className="flex items-center justify-between group cursor-pointer" onClick={() => handleSort(k)}>
                    <div className="flex items-center hover:text-white transition">
                        {children}{sortArrow(k)}
                    </div>
                    {filterType && (
                        <div 
                            className={`ml-1 px-1 rounded hover:bg-slate-700 transition ${isActive ? 'text-blue-400 bg-blue-900/30' : 'text-slate-600 opacity-0 group-hover:opacity-100'}`}
                            onClick={(e) => toggleFilterMenu(e, k)}
                            title="Filter Column"
                        >
                            ▼
                        </div>
                ...
                {showMenu && (
                    <div className="absolute top-full left-0 mt-1 bg-slate-800 border border-slate-600 shadow-2xl rounded p-3 z-50 text-slate-200 text-xs shadow-black/50 min-w-[150px] font-normal" onClick={e => e.stopPropagation()}>
                        <div className="font-bold border-b border-slate-700 pb-2 mb-2 flex justify-between items-center text-[11px] uppercase tracking-wider">
                            <span>Filter {k.replace('_', ' ')}</span>
                            <button onClick={(e) => toggleFilterMenu(e, null)} className="text-slate-400 hover:text-white px-1">✕</button>
                        </div>
                        
                        {filterType === 'category' && (
                            <div className="flex flex-col gap-1 max-h-40 overflow-y-auto pr-1">
                                {filterOptions.map(opt => (
                                    <label key={opt} className="flex items-center gap-2 cursor-pointer hover:bg-slate-700 p-1 rounded transition select-none">
                                        <input 
                                            type="checkbox" 
                                            checked={(currentFilter.values || []).includes(String(opt))}
                                            onChange={(e) => {
                                                const vals = new Set(currentFilter.values || []);
                                                if (e.target.checked) vals.add(String(opt));
                                                else vals.delete(String(opt));
                                                applyFilter(k, { type: 'category', values: Array.from(vals) });
                                            }}
                        ...
                        {filterType === 'range' && (
                            <div className="flex flex-col gap-2">
                                <div className="flex items-center gap-2">
                                    <span className="w-8 text-slate-400">Min:</span>
                                    <input 
                                        type="number" 
                                        value={currentFilter.min !== undefined ? currentFilter.min : ''}
                                        onChange={e => applyFilter(k, { type: 'range', min: e.target.value, max: currentFilter.max !== undefined ? currentFilter.max : '' })}
                                        className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1 outline-none focus:border-blue-500"
                                    />
                                </div>
                                <div className="flex items-center gap-2">
                                    <span className="w-8 text-slate-400">Max:</span>
                                    <input 
                                        type="number" 
                                        value={currentFilter.max !== undefined ? currentFilter.max : ''}
                                        onChange={e => applyFilter(k, { type: 'range', min: currentFilter.min !== undefined ? currentFilter.min : '', max: e.target.value })}
                                        className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1 outline-none focus:border-blue-500"
                                    />
                                </div>
                            </div>
                        ...
                        {isActive && (
                            <button onClick={(e) => clearFilter(e, k)} className="mt-3 w-full bg-red-900/20 text-red-400 hover:bg-red-800 hover:text-white border border-red-900/50 rounded py-1.5 transition font-bold">
                                Clear Filter
                            </button>
                        )}
                    </div>
    ...
    const renderTable = (data) => (
        ...
        <div className="overflow-hidden rounded-xl border border-slate-700 shadow-xl shadow-slate-900/20">
            ...
            <div className="bg-slate-800">
                ...
                <table className="w-full text-left" style={{ tableLayout: 'fixed' }}>
                    ...
                    <colgroup>
                        ...
                        <col style={{ width: '4.5%' }} />{/* Ticker */}
                        <col style={{ width: '4%' }} />{/* 3M */}
                        <col style={{ width: '4%' }} />{/* 2M */}
                        <col style={{ width: '4%' }} />{/* 1M */}
                        <col style={{ width: '4%' }} />{/* 1W */}
                        ...
                        <col style={{ width: '4.5%' }} />{/* SMI */}


                        <col style={{ width: '4.5%' }} />{/* MACD */}


                        <col style={{ width: '4.5%' }} />{/* M.MACD */}


                        <col style={{ width: '5%' }} />{/* EMA60 */}


                        <col style={{ width: '5.5%' }} />{/* SMA200 */}


                        <col style={{ width: '7%' }} />{/* DMI */}


                        <col style={{ width: '4.5%' }} />{/* Vol */}


                        <col style={{ width: '5%' }} />{/* Vol W/M */}


                        <col style={{ width: '4.5%' }} />{/* Stage */}


                        <col style={{ width: '5%' }} />{/* 52W% */}
                        <col style={{ width: '4.5%' }} />{/* VCP */}


                        <col style={{ width: '6%' }} />{/* DI Stack */}


                        <col style={{ width: '5%' }} />{/* Score */}


                        <col style={{ width: '4.5%' }} />{/* UV */}


                        <col style={{ width: '3.5%' }} />{/* Stars */}


                        <col style={{ width: '3%' }} />{/* Go */}


                    </colgroup>


                    <thead className="bg-slate-900 text-slate-400 uppercase font-bold text-[8px]">


                        <tr>


                            <Th k="ticker" className="text-left w-12 cursor-pointer group">Tkr</Th>
                            <Th k="perf_3m" className="text-right" filterType="range">3M%</Th>
                            <Th k="perf_2m" className="text-right" filterType="range">2M%</Th>
                            ...
                            <Th k="macd_m" className="text-right" filterType="range">M.MACD</Th>
                            <Th k="ema60_d" className="text-right" filterType="range">EMA60</Th>
                            <Th k="sma200_d" className="text-right" filterType="range">SMA200</Th>
                            <Th k="di_plus" className="text-center" filterType="range">DMI</Th>
                            <Th k="vol_ratio" className="text-right" filterType="range">Vol</Th>
                            <Th k="vol_week_vs_month" className="text-right" filterType="range">W/M</Th>
                            <Th k="stage" className="text-center text-purple-400" filterType="category" filterOptions={['1', '2', '3', '4', '0']}>Stg</Th>
                            <Th k="range_52w" className="text-right" filterType="range">52W%</Th>
                            <Th k="vcp_tightness" className="text-right" filterType="range">VCP</Th>
                            <Th k="di_plus" className="text-center text-blue-400" filterType="range">DI Stk</Th>
                            <Th k="momentum_score" className="text-center text-green-400" filterType="range">Score</Th>
                            <Th k="udvr" className="text-center text-cyan-400" filterType="range">UV</Th>
                            <Th k="stars" className="text-center text-orange-400" filterType="category" filterOptions={['⭐⭐⭐', '⭐⭐', '⭐']}>★</Th>


                            <th className="px-1 py-1.5 text-center">Go</th>
                    ...
                    <tbody className="divide-y divide-slate-700/50">
                        ...
                        {data.map((row, idx) => {
                            ...
                            const isWL = isWatchlistCandidate(row);


                            return (


                                <tr key={idx} className={`hover:bg-slate-700/50 transition duration-150 ${isWL ? 'border-l-2 border-l-amber-500' : ''}`}>
                                    ...
                                    <td className={`px-1 py-1 text-right font-mono text-[9px] ${row.perf_3m > 0 ? 'text-green-400' : row.perf_3m < 0 ? 'text-red-400' : 'text-slate-500'}`}>
                                        ...
                                        {row.rally_high_3m != null && (
                                            <div className="text-[7px] text-slate-500 mt-0.5 whitespace-nowrap">
                                                L:{row.rally_low_3m.toFixed(1)} H:{row.rally_high_3m.toFixed(1)}
                                            </div>
                                        )}
                                    </td>
                                    <td className={`px-1 py-1 text-right font-mono text-[9px] ${row.perf_2m > 0 ? 'text-green-400' : row.perf_2m < 0 ? 'text-red-400' : 'text-slate-500'}`}>
                                        {row.perf_2m != null ? `${row.perf_2m > 0 ? '+' : ''}${row.perf_2m.toFixed(1)}%` : '-'}
                                    </td>
                                    <td className={`px-1 py-1 text-right font-mono text-[9px] ${row.perf_1m > 0 ? 'text-green-400' : row.perf_1m < 0 ? 'text-red-400' : 'text-slate-500'}`}>
                                        {row.perf_1m != null ? `${row.perf_1m > 0 ? '+' : ''}${row.perf_1m.toFixed(1)}%` : '-'}
                                    </td>
                                    <td className={`px-1 py-1 text-right font-mono text-[9px] ${row.perf_1w > 0 ? 'text-green-400' : row.perf_1w < 0 ? 'text-red-400' : 'text-slate-500'}`}>
                                        {row.perf_1w != null ? `${row.perf_1w > 0 ? '+' : ''}${row.perf_1w.toFixed(1)}%` : '-'}
                                    </td>
                                    ...
                                    <td className="px-1 py-1 text-center">
                                        ...
                                        {(() => {
                                            ...
                                            const colorMap = { green: 'bg-green-500', blue: 'bg-blue-500', yellow: 'bg-yellow-500', orange: 'bg-orange-500', pink: 'bg-pink-400', red: 'bg-red-500', gray: 'bg-slate-600' };
                                            ...
                                            return <span className={`inline-block px-1 py-0.5 rounded text-[8px] font-bold text-white ${colorMap[c]}`}>{labelMap[c]}</span>;
                                    ...
                                    <td className={`px-1 py-1 text-right font-bold text-[10px] ${row.smi_bullish ? 'text-green-400' : 'text-red-400'}`}>
                                    ...
                                    <td className="px-1 py-1 text-right text-[10px] font-mono">


                                        <span className={row.macd_d > 0 ? 'text-green-400' : 'text-red-400'}>{row.macd_d?.toFixed(2)}</span>
                                    ...
                                    <td className="px-1 py-0.5 text-right font-mono">
                                        {row.macd_m ? (
                                            <div className="flex flex-col items-end leading-none justify-center h-full pt-1">
                                                <span className={`text-[10px] ${row.macd_m.is_positive ? 'text-green-400 font-bold' : 'text-red-400 font-bold'}`}>
                                                    {row.macd_m.macd?.toFixed(2)}
                                                </span>
                                                <span className={`text-[7px] mt-0.5 ${row.macd_m.is_positive ? 'text-green-500/80' : 'text-red-500/80'}`}>
                                                    {row.macd_m.is_positive ? '(+)' : '(-)'}
                                                </span>
                                            </div>
                                        ) : (
                                    ...
                                    <td className={`px-1 py-1 text-right font-mono text-[10px] ${row.price > row.ema60_d ? 'text-green-400' : 'text-slate-500'}`}>
                                    ...
                                    <td className={`px-1 py-1 text-right font-mono text-[10px] ${row.sma200_d && row.price > row.sma200_d ? 'text-green-400' : 'text-red-400'}`}>
                                    ...
                                    <td className="px-1 py-1 text-center">
                                        ...
                                        <div className={`text-[9px] font-bold ${row.is_bullish ? 'text-green-400' : 'text-slate-500'}`}>
                                        ...
                                        <div className="text-[8px] text-slate-500 font-mono">{row.di_plus?.toFixed(0)}/{row.di_minus?.toFixed(0)}/{row.adx?.toFixed(0)}</div>


                                    </td>


                                    <td className={`px-1 py-1 text-right font-bold text-[10px] ${row.is_vol_growing ? 'text-orange-400' : 'text-slate-500'}`}>
                                    ...
                                    <td className={`px-1 py-1 text-right font-bold text-[10px] ${row.vol_week_vs_month > 1.1 ? 'text-green-400' : row.vol_week_vs_month > 1.0 ? 'text-yellow-400' : row.vol_week_vs_month < 0.9 ? 'text-red-400' : 'text-slate-500'}`}>
                                    ...
                                    <td className="px-1 py-1 text-center">
                                        ...
                                        {(() => {
                                            ...
                                            if (!stage || stage.stage === 0) return <span className="text-slate-600 text-[9px]">-</span>;


                                            const colors = { blue: 'bg-blue-500/20 text-blue-400 border-blue-500/30', green: 'bg-green-500/20 text-green-400 border-green-500/30', yellow: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30', red: 'bg-red-500/20 text-red-400 border-red-500/30', gray: 'bg-slate-500/20 text-slate-400 border-slate-500/30' };


                                            return <span className={`px-1 py-0.5 rounded text-[8px] font-bold border ${colors[stage.color] || colors.gray}`} title={stage.label}>S{stage.stage}</span>;
                                    ...
                                    <td className="px-1 py-1 text-right">
                                        ...
                                        {(() => {
                                            ...
                                            const pct = row.high_52w_dist_pct;


                                            if (pct == null) return <span className="text-slate-600 text-[9px]">-</span>;


                                            const color = pct <= 10 ? 'text-green-400' : pct <= 25 ? 'text-yellow-400' : 'text-red-400';


                                            return <span className={`font-bold text-[10px] ${color}`}>{pct.toFixed(0)}%</span>;
                                    ...
                                    <td className="px-1 py-1 text-right">
                                        {(() => {
                                            const vcp = row.vcp_metrics;
                                            if (!vcp || vcp.tightness_pct === 0) return <span className="text-slate-600 text-[9px]">-</span>;
                                            const tight = vcp.tightness_pct;
                                            let color = 'text-slate-500';
                                            if (tight <= 5.0) color = 'text-green-400';
                                            else if (tight <= 10.0) color = 'text-yellow-400';
                                            else if (tight > 20.0) color = 'text-red-400';

                                            return (
                                                <div className="flex flex-col items-end">
                                                    <span className={`font-bold text-[10px] ${color}`}>{tight.toFixed(1)}%</span>
                                                    <span className="text-[8px] text-slate-500">{vcp.contractions}T</span>
                                                </div>
                                            );
                                        })()}
                                    </td>
                                    ...
                                    <td className="px-1 py-1 text-center">
                                        ...
                                        {(() => {
                                            ...
                                            if (!di) return <span className="text-slate-600 text-[9px]">-</span>;
                                            ...
                                            return (
                                                ...
                                                <div className="flex gap-px justify-center">
                                                    ...
                                                    <span className={`px-0.5 py-0.5 rounded text-[7px] font-bold ${di.h1 === true ? 'bg-green-500/20 text-green-400' : di.h1 === false ? 'bg-red-500/20 text-red-400' : 'bg-slate-500/20 text-slate-500'}`}>1</span>


                                                    <span className={`px-0.5 py-0.5 rounded text-[7px] font-bold ${di.h4 === true ? 'bg-green-500/20 text-green-400' : di.h4 === false ? 'bg-red-500/20 text-red-400' : 'bg-slate-500/20 text-slate-500'}`}>4</span>


                                                    <span className={`px-0.5 py-0.5 rounded text-[7px] font-bold ${di.d1 === true ? 'bg-green-500/20 text-green-400' : di.d1 === false ? 'bg-red-500/20 text-red-400' : 'bg-slate-500/20 text-slate-500'}`}>D</span>
                                    ...
                                    <td className="px-1 py-1 text-center">
                                        ...
                                        {(() => {
                                            ...
                                            if (score === undefined || score === null) return <span className="text-slate-600 text-[9px]">-</span>;
                                            ...
                                            return <span className={`font-bold text-[10px] ${colorClass}`}>{score}</span>;
                                    ...
                                    <td className="px-1 py-1 text-center">
                                        ...
                                        {(() => {
                                            ...
                                            if (!pg) return <span className="text-slate-600 text-[9px]">-</span>;
                                            ...
                                            if (uv >= 60) colorClass = 'text-green-400';


                                            else if (uv >= 55) colorClass = 'text-lime-400';


                                            else if (uv <= 40) colorClass = 'text-red-400';
                                            ...
                                            return <span className={`font-bold text-[9px] ${colorClass}`}>{uv?.toFixed(0)}{trend && <span className="text-[7px]">{trend}</span>}</span>;
                                    ...
                                    <td className="px-1 py-1 text-center">
                                        ...
                                        <button
                                            ...
                                            className="bg-blue-600/20 hover:bg-blue-600 text-blue-400 hover:text-white px-1.5 py-0.5 rounded text-[9px] transition border border-blue-600/30"
        ...
        <div className="p-4 animate-fade-in">
            ...
            {stats && (
                ...
                <div className="grid grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 border-amber-700/50">


                        <div className="text-amber-400 text-xs uppercase font-bold">⭐ Watchlist</div>
            ...
            {!stats && !scanning && (
                ...
                <div className="text-center py-20 border-2 border-dashed border-slate-800 rounded-xl">
                    ...
                    <p className="text-slate-400 mt-2">Press "Run Scan" to find weekly reversal setups.</p>
                ...
                <div>
                    ...
                    <div className="flex gap-2 mb-4">
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition ${scannerTab === 'all' ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/40' : 'bg-slate-800 text-slate-400 hover:text-white border border-slate-700'}`}
                        >
                            📊 All Results ({results.length})
                        </button>
                        <button
                            onClick={() => setScannerTab('watchlist')}
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition flex items-center gap-1.5 ${scannerTab === 'watchlist' ? 'bg-amber-600 text-white shadow-lg shadow-amber-900/40' : 'bg-slate-800 text-slate-400 hover:text-white border border-slate-700'}`}
                        >
                            ⭐ Watchlist Candidates ({watchlistCount})
                        </button>
                    </div>

                    {/* Watchlist criteria hint */}
                    {scannerTab === 'watchlist' && (
                        <div className="mb-3 text-[10px] text-slate-500 bg-slate-800/50 border border-slate-700/50 px-3 py-1.5 rounded-lg inline-flex gap-3">
                            <span>RSI(W) ≥ 48</span>
                            <span>Phase: Strong/Accum/Pull/Corr</span>
                        </div>
                    )}
                    ...
                    {sortedResults.length > 0 ? (
                        ...
                        <div className="p-12 text-center bg-slate-800/50 rounded-xl border border-dashed border-slate-700">
                            <h3 className="text-slate-400 font-bold mb-2">No Matches Found</h3>
                            <p className="text-slate-500 text-sm">No tickers meet the strict criteria for this view.</p>
                            {scannerTab === 'watchlist' && (
                                <button onClick={() => setScannerTab('all')} className="mt-4 text-sm text-blue-400 hover:text-blue-300 transition">
                                    ← Back to All Results
                                </button>
                            )}
                        </div>
                        ...
                        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            ...
                            <div className={`rounded-xl p-4 border-l-4 shadow-lg flex items-center gap-4 ${optionResult.analysis?.comp_signal.includes('NO OPERAR') ? 'bg-red-900/20 border-red-500' :
                                ...
                                <div className={`text-3xl ${optionResult.analysis?.comp_signal.includes('NO OPERAR') ? 'text-red-500' :
                                    ...
                                    {optionResult.analysis?.comp_signal.includes('NO OPERAR') ? '⛔' : optionResult.analysis?.comp_signal.includes('RIESGO') ? 'âš ï¸' : '🚀'}
            ...
            <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700 mb-6">
                ...
                <div className="flex flex-wrap gap-4 items-end">
                    ...
                    >
                        ...
                        {loading ? '⏳ Scanning...' : 'ðŸ” Scan Portfolio'}
...
function ChatCopilot() {
    ...
    const handleSend = async (text = null) => {
        ...
        try {
            ...
            setMessages([...newMessages, { role: 'assistant', content: "âŒ Network error. Please check your connection." }]);
        ...
        <div className="flex h-screen overflow-hidden selection:bg-blue-500/30 selection:text-white">
            ...
            <div className={`w-[70px] sm:w-[240px] flex-shrink-0 border-r border-[#1a1a1a] flex flex-col transition-all duration-300 z-50 ${selectedTicker ? 'hidden md:flex' : 'flex'}`}>
                ...
                <nav className="flex-1 overflow-y-auto p-2 space-y-0.5 custom-scrollbar">
                    ...
                    <button onClick={() => setView('mentor')} className={`group relative w-full flex items-center p-2 rounded-xl transition-all duration-200 ${view === 'mentor' ? 'bg-indigo-600/10 text-white' : 'hover:bg-[#151515] text-slate-400 hover:text-white'}`}>


                        <span className={`text-lg ${view === 'mentor' ? 'text-indigo-400' : 'text-slate-500 group-hover:text-indigo-400'}`}>🎓</span>


                        <span className="ml-3 text-sm font-medium hidden sm:block">Mentor</span>


                    </button>


                    <button onClick={() => setView('mentor-scanner')} className={`group relative w-full flex items-center p-2 rounded-xl transition-all duration-200 ${view === 'mentor-scanner' ? 'bg-purple-600/10 text-white' : 'hover:bg-[#151515] text-slate-400 hover:text-white'}`}>


                        <span className={`text-lg ${view === 'mentor-scanner' ? 'text-purple-400' : 'text-slate-500 group-hover:text-purple-400'}`}>🎯</span>
                ...
                <main className="flex-1 overflow-y-auto relative custom-scrollbar">
                    ...
                    {!selectedTicker ? (
                        ...
                        <div className="min-h-full">
                            ...
                            <div style={{ display: view === 'mentor' ? 'block' : 'none' }}>


                                {visitedViews.has('mentor') && <MomentumMentorView />}


                            </div>


                            <div style={{ display: view === 'mentor-scanner' ? 'block' : 'none' }}>


                                {visitedViews.has('mentor-scanner') && <MentorScannerView />}
