...
def generate_expert_summary(indices, sectors):
    ...
    # Fetch market headlines (async-safe, cached)
    headlines = []
    try:
        import news
        raw_headlines = news.get_market_headlines(days=1)
        headlines = [{"headline": h.get("headline", ""), "ticker": h.get("ticker", ""), "sentiment": h.get("sentiment", "neutral")} for h in raw_headlines[:3]]
    except Exception as e:
...
def get_economic_calendar():
    """ Returns upcoming high-impact economic events. Dynamic generation. """
    today = datetime.now()
    month = today.strftime("%b")
    
    # Simple logic to project "Next Thursday" etc.
    return [
        {"event": "CPI Inflation Data", "date": f"{month} 14", "time": "08:30 AM", "impact": "High"},
        {"event": "Initial Jobless Claims", "date": f"{month} 16", "time": "08:30 AM", "impact": "Medium"},
        {"event": "FOMC Rate Decision", "date": f"{month} 29", "time": "02:00 PM", "impact": "High"},
        {"event": "Non-Farm Payrolls (NFP)", "date": f"{month} 03", "time": "08:30 AM", "impact": "High"}
    ]
            ...
            try:
                ...
                if btc_data is not None and not btc_data.empty:
                    ...
                    if len(btc_close) >= 2:
                        ...
                        prev_btc = float(btc_close.iloc[-2])
                        btc_chg = ((current_btc - prev_btc) / prev_btc) * 100 if prev_btc > 0 else 0
                        status["BTC"] = {
